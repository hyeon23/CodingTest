...
#define Y second
string board[1001];
int dist1[1001][101] = { -1, };//불 이동거리
int dist2[1001][101] = { -1, };//지훈 이동거리
int n, m;
int dx[4] = {1, 0, -1, 0};
int dy[4] = {0, 1, 0, -1};
int main(){
    ...
    queue<pair<int, int>> Q1;//불큐
    queue<pair<int, int>> Q2;//지훈 큐
    for(int i = 0; i < n; ++i){
        for(int j = 0; j < m; ++j){
            if(board[i][j] == 'F'){
                Q1.push({i, j});
    ...
    while(!Q1.empty()){
        ...
        for(int dir = 0; dir < 4; ++dir){
            ...
            if(dist1[nx][ny] >= 0/*방문 체크*/ || board[nx][ny] == '#'/*벽 체크*/) continue;
    ...
    while(!Q2.empty()){
        ...
        for(int dir = 0; dir < 4; ++dir){
            ...
            if(dist2[nx][ny] >= 0/*방문 체크*/ || board[nx][ny] == '#'/*벽 체크*/) continue;
            if(dist1[nx][ny] != -1 && dist1[nx][ny] <= dist2[cur.X][cur.Y] + 1/*불이 이미 전파된 경우*/) continue;
